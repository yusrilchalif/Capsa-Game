...
public class Straight : IEvaluator<Straight>
{
	...
	public static void EvaluateStraight(ref List<PokerWin> results, Card card, ref CardSet bucket, ref CardSet special, Func<Card, bool> filter)
	{
		if (bucket.Count > 0)
		{
			...
			{
				if (filter(bucket[4]))
					results.Add(new PokerWin(new CardSet(bucket), bucket[4], PokerWin.CombinationType.Straight));
				bucket.RemoveAt(0);
			}
		}
		else
	...
	public static void PostEvaluateStraight(ref List<PokerWin> results, ref CardSet bucket, ref CardSet special)
	{
		if (special.Count >= 5 && special[special.Count - 1].Nominal == "2")
		{
			if (special.Count == 5)
			{
				results.Add(new PokerWin(special, special[4], PokerWin.CombinationType.Straight));
			}
			else if (special.Count == 6)
			{
				...
				results.Add(new PokerWin(set, set[0], PokerWin.CombinationType.Straight));

				set = special.GetRange(special.Count - 2, 2); // A-2
				set.AddRange(special.GetRange(0, 3)); // 3-4-5
				results.Add(new PokerWin(set, set[1], PokerWin.CombinationType.Straight));
			}
		}
	}
