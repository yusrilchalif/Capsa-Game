...
public class TrickController : MonoBehaviour
{
	...
	List<PokerWin> tricks = new List<PokerWin>();
	TrickView view;
	int nextTurnPlayer;
	int lastTurnPlayer;
	int passPlayer;
	...
	void Start()
	{
		...
		{
			int avatarId = GameManager.Instance.avatarPlayerId;
			avatarId = avatarId + i >= players.Count ? Mathf.Abs(avatarId - i) : avatarId + i;
			players[i].id = i;
			players[i].Cards = set.GetRange(i * 13, 13);
			players[i].playerEmotion.Avatar = GameManager.Instance.avatars[avatarId];
	...
	public bool Deal(PokerWin hand)
	{
		...
		if (hand.Cards.Count == 0 || hand.Combination == PokerWin.CombinationType.Invalid)
		{
		...
		else
		{
			...
			if (hand.Combination <= PokerWin.CombinationType.Triple && last.Combination <= PokerWin.CombinationType.Triple)
			{
			...
			else if (hand.Combination > PokerWin.CombinationType.Triple && last.Combination > PokerWin.CombinationType.Triple)
			{
				// 5-card hand
				if (hand > last)
				{
